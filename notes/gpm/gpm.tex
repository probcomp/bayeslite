...
\item \texttt{$\vec{s}_i$ =
    simulate($\mathcal{G}$, row = $i$, targets = $\set{c_j}_{j=1}^{|T|}$, givens
    ...
    \set{ \mathbf{X}_{ik} = x_{ik} }, \mathcal{G}).
    $$

    The returned $\vec{s}$ is a $|T|$ dimensional vector.

\item \texttt{$\log p$ =
    logpdf($\mathcal{G}$, row = $i$, targets = $\set{(c_j, x_{ij})}_{j=1}^{|T|}$,
...
columns in the \texttt{typed-outputs} of the GPM $\mathcal{G}$, and
the \texttt{given} indexes of each query must be a superset of the
\texttt{typed-inputs} of the GPM $\mathcal{G}$.  Note: A client may
...
\item $\set{I_k} = \texttt{CMI}(
    ...
    \begin{align*}
    {\tt logpdf(} \mathcal{G}, {\tt targets} = \set{(r^*,c_i^a,\hat{a}_{j,i})}
...
\item $p = \texttt{marginal-dependence-prob}(\mathcal{G},
    \texttt{colA}=c_i,
    \texttt{colB}=c_j,
    \texttt{accuracy}=N)$
...
\item $r^2 = \texttt{generative-similarity}(\mathcal{G},
