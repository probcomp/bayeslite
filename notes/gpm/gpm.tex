...
Linear regression (with a fixed model $p_o(.|.)$ for the output noise) can be viewed as a
conditional adaptive GPM over the class of non-adaptive GPMs
parameterized by the possible regression coefficients $\vec\beta$.
For a given set of input columns, defining inputs $\vec x$, and output column $y$,
the adaptation rule corresponding to maximum likelihood estimation would be

\[ \vec\beta^*|\{\vec x_i, y_i\} = \argmax_{\vec\beta^*} \sum_i
   p_o(y_i | \vec x_i^T \vec\beta^*). \]

The {\tt simulate} and {\tt logpdf} methods would refer to the
noise distribution around the predicted value for $y$, namely $p_o(.|\vec x^T \vec \beta)$.
\end{example}
...
$\Pi(\G_\alpha)$ over
$\{\G_\alpha\}$, and adaptation by Bayes' rule
...
\todo[inline]{Find a place for the ideas contained in this chunk of
  prize-winning text, or flush it (see also the next section):}
...
\todo[inline]{Find a place for the ideas contained in this chunk of
  prize-winning text, or flush it (see also the previous section):}
