...
if [ -n "${TRAVIS_TAG}" ]; then
  ...
  # if tag didn't come from master, add the "experimental" label
  if [ $(git branch --contains tags/${TRAVIS_TAG}) = "master" ]; then
    ...
    anaconda -t ${CONDA_UPLOAD_TOKEN} upload -u ${CONDA_USER} -l experimental ~/miniconda/conda-bld/linux-64/bayeslite-*.tar.bz2 --force
  fi
